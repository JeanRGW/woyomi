...
engine.registerSource(reg.sources[0])

const query = process.argv[2] ?? 'berserk'
const results = await engine.search('mangadex', query, 1)
...
const episodes = await engine.getEpisodes('mangadex', media.mediaId)
console.log(`episodes: ${episodes.length}`)
const chapter = episodes.find((e) => Number.isInteger(e.number) && e.number >= 1)
if (!chapter) throw new Error('no normal chapter')

const content = await engine.getChapterContent('mangadex', media.mediaId, chapter.id)
